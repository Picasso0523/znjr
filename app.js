...
const CONVERSATION_STORAGE_KEY_PREFIX = "anyihua_coze_conversation_";
let chatHistory = [];
let isBotTyping = false;
let typingMessageEl = null;

function getApiBase() {
  const fromWindow = typeof window !== "undefined" ? window.__AI_API_BASE__ : "";
  if (fromWindow && typeof fromWindow === "string") return fromWindow.replace(/\/$/, "");
  return "https://xiaoma-kefu.1063528814.workers.dev";
}
...
function getConversationId() {
  try {
    const key = CONVERSATION_STORAGE_KEY_PREFIX + getUserId();
