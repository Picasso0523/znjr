...
if (aiForm && aiInput) {
  ...
  aiForm.addEventListener("submit", async (e) => {
    ...
    try {
      // 调用后端转发到 Coze 的接口（Cloudflare Worker），后端负责使用 token 和 appId
      const resp = await fetch("https://xiaoma-kefu.bijiasuo0523.workers.dev/api/chat", {
        method: "POST",
        headers: {
      ...
      const answer = data.answer || "当前暂时无法回答，请稍后重试。";
      appendMessage(answer, "bot");
    } catch (err) {
      appendMessage("抱歉，服务繁忙，请稍后再试。", "bot");
    }
  });
}
